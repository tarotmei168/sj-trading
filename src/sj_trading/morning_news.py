...
# 關鍵字 → 顏色分類
# ═══════════════════════════════════════════════════
# 新聞關鍵字架構表：docs/NEWS_KEYWORD_FRAMEWORK.md
# 修改關鍵字時，必須同步更新架構表 + 兩個 Python 檔
# ═══════════════════════════════════════════════════
...
KEYWORD_RULES = [
    ...
    # 動能反轉 (紅色)
    (['拋售', '動能股', '觸底', '回補', '落底'], '🔴動能反轉'),
    # 匯率 (紅色)
    (['台幣', '重貶', '貶值', '32.'], '🔴匯率'),
    # AMD/超微 (紅色)
    (['AMD', '超微', '大單', 'CPU', 'GPU', '伺服器'], '🔴AMD/超微'),
    # 產業大事 (橙色)
    (['晶片', 'chip', '半導體', '台積電', 'TSMC'], '🟠半導體'),
    (['AI', '人工智能', '人工智慧'], '🟠AI'),
    (['WAIC', '智能體', '具身智慧', 'AI落地'], '🟠AI應用'),
    (['法說', '法說會'], '🟠法說'),
    (['營收', '財報'], '🟠財報'),
    (['除息', '除權'], '🟠除息'),
    (['ASML', '算力', '荷蘭ASML', '曝光機'], '🟠半導體設備'),
